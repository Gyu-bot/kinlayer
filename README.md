...
대화 후 쓰기 흐름은 보수적으로 동작해야 합니다. 이름이 명확하면 먼저 entity resolve로 기존 사람을 찾고, AI가 추론한 새 정보는 candidate로 제출합니다. `그 사람`처럼 현재 턴만으로 식별할 수 없는 표현은 새 사람을 만들지 않고 clarification으로 남깁니다. `Alex는 직장 동료가 아니라 사촌이야`처럼 사용자가 명시적으로 정정한 경우만, 대상 record가 하나로 확정될 때 correction apply를 사용합니다. 공인/뉴스 인물, 허구 예시, 일반 직군/집단 언급은 Kinlayer 관계 기억으로 쓰지 않습니다.
